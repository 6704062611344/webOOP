...
abstract class Animal implements IDisplayable {
    ...
    public function __construct($data = []) {
        ...
        $this->conservationStatus = $data['conservation_status'] ?? '';
    ...
    public function getConservationStatus() { return $this->conservationStatus; }
...
// ============================================
// WILDLIFE ANIMAL ABSTRACT CLASS (Inheritance)
// ============================================

abstract class WildlifeAnimal extends Animal implements IVotable {
    protected $sound;
    protected static $db;
...
class Mammal extends WildlifeAnimal {
    private $subtype;
    ...
    public function move() {
        switch ($this->subtype) {
            case 'FlyingSquirrel':
                return "โผบินระหว่างต้นไม้ด้วยเยื่อบิน";
            case 'Elephant':
            case 'Tiger':
            case 'Bear':
                return "เดินเป็นสี่เท้าบนพื้นดิน";
            case 'Deer':
                return "วิ่งกระโดดอย่างรวดเร็ว";
            default:
                return "เคลื่อนไหวแบบสัตว์เลี้ยงลูกด้วยนม";
        }
    }
}

// คลาสนก
class Bird extends WildlifeAnimal {
    private $subtype;
    
    public function __construct($data = []) {
        parent::__construct($data);
        $this->subtype = $data['subtype'] ?? 'General Bird';
    }
    
    public function getAnimalType() {
        return "นก ({$this->subtype})";
    }
    ...
    public function move() {
        switch ($this->subtype) {
            ...
            case 'Parrot':
            case 'Hornbill':
                return "บินด้วยปีกที่แข็งแรง สามารถโฉบเฉี่ยวได้อย่างรวดเร็ว";
            case 'Crane':
                return "บินระยะไกลได้ เดินและว่ายน้ำเก่ง";
            default:
                return "บินและกระโดดระหว่างกิ่งไม้";
        }
    }
}

// คลาสสัตว์เลื้อยคลาน
class Reptile extends WildlifeAnimal {
    private $subtype;
    
    public function __construct($data = []) {
        parent::__construct($data);
        $this->subtype = $data['subtype'] ?? 'General Reptile';
    }
    
    public function getAnimalType() {
        return "สัตว์เลื้อยคลาน ({$this->subtype})";
    }
    ...
    public function move() {
        switch ($this->subtype) {
            case 'Python':
                return "เลื้อยไปด้วยการคดเคี้ยวตัว มีความยืดหยุ่นสูง";
            case 'Crocodile':
                return "เลื้อยและว่ายน้ำด้วยหาง มีความแข็งแรงมาก";
            case 'Turtle':
                return "เดินช้าๆ บนบกและว่ายน้ำด้วยขาหน้า";
            case 'Monitor':
                return "เดินและว่ายน้ำได้ดี สามารถปีนต้นไม้ได้";
            default:
                return "เลื้อยและคลานบนพื้นดิน";
        }
    }
}

// คลาสสัตว์สะเทินน้ำสะเทินบก
class Amphibian extends WildlifeAnimal {
    private $subtype;
    
    public function __construct($data = []) {
        parent::__construct($data);
        $this->subtype = $data['subtype'] ?? 'General Amphibian';
    }
    
    public function getAnimalType() {
        return "สัตว์สะเทินน้ำสะเทินบก ({$this->subtype})";
    }
    ...
    public function move() {
        switch ($this->subtype) {
            case 'Frog':
            case 'Toad':
                return "กระโดดด้วยขาหลังที่แข็งแรง ว่ายน้ำเก่ง";
            case 'Salamander':
                return "เดินคลานบนพื้นและว่ายน้ำด้วยหาง";
            default:
                return "เคลื่อนไหวทั้งบนบกและในน้ำ";
        }
    }
}
...
class Database {
    ...
    private function __construct() {
        ...
        $dbname = 'if0_40300999_01';
        $username = 'if0_40300999';
        $password = 'tyvvYINlKA';
